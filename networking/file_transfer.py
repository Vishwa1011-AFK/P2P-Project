...
class FileTransfer:
    ...
    async def pause(self):
        async with self.condition:
            if self.state == TransferState.IN_PROGRESS:
                ...
                logger.warning(f"Cannot pause transfer {self.transfer_id[:8]} - not in progress (state: {self.state})")
                return False
    ...
    async def resume(self):
        ...
        if self.state == TransferState.PAUSED:
            self.state = TransferState.IN_PROGRESS
            logger.info(f"Transfer {self.transfer_id[:8]} ({self.direction}) resumed from {old_state}.")
            await message_queue.put({"type": "transfer_update"})
                ...
                if current_state == TransferState.PAUSED:
                    ...
                    try:
                        ...
                        async with transfer.condition:
                            while transfer.state == TransferState.PAUSED and not shutdown_event.is_set():
                                try:
                                    ...
                                    logger.debug(f"Woke up from pause wait for {transfer_id[:8]}, new state: {transfer.state}")
                                    ...
                                    if websocket.state != State.OPEN:
                                        ...
                                        break
                                    logger.debug(f"Timeout while waiting for resume - still paused: {transfer.state == TransferState.PAUSED}")
                                    continue
                            if transfer.state == TransferState.FAILED:
                                break
                        if transfer.state == TransferState.IN_PROGRESS and not shutdown_event.is_set():
                            ...
                            # Wait a moment to ensure receiver has processed the pause completely
                            ...
                            # Wait another moment for the resume message to be processed
                            await asyncio.sleep(0.5)
...
async def update_transfer_progress():
    while not shutdown_event.is_set():
        try:
            ...
            updated = False  # Initialize the updated variable
