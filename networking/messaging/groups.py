...
async def send_group_invite_message(groupname, peer_ip):
    ...
    if ws and ws.state == State.OPEN:
        await ws.send(message)
        pending_invites[peer_ip].add((groupname, own_ip))
        await message_queue.put(f"Sent invite to {peer_ip} for '{groupname}'")
...
async def send_group_update_message(groupname, members):
    message = json.dumps({"type": "GROUP_UPDATE", "groupname": groupname, "members": list(members)})
